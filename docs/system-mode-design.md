...
This artifact is for local verification only and is published with `v0.2.0-preview.1` as a public prerelease artifact. It is not a production installer.
...
Machine rules may also carry `coreReserve`. The default is `0`, which preserves current affinity behavior. A nonzero value requires Windows physical-core topology and applies a process affinity mask that leaves the requested number of physical cores unused for the matched process. PriorityGear preserves the raw Windows `PROCESSOR_RELATIONSHIP.EfficiencyClass` value. All-zero values mean the system is homogeneous or does not expose a distinction; if any nonzero values are present, higher values are treated as higher-performance cores and reserved first. If topology is unavailable, the reserve count is invalid, multiple processor groups are required, or affinity cannot be applied to the target process, the service reports an explicit failure rather than silently continuing as if Core Reserve worked.
...
Service process discovery groups running services by host PID and reports service names, display names where available, process name/path where readable, current priority where readable, shared-host status, and priority-access probe status. The implementation uses the SCM `EnumServicesStatusEx` API to obtain service PIDs without shelling out once per service.
...
Targeted service-name and PID discovery inspect only the target process for path/priority/access details. They enumerate service PID mappings to determine the host group, but they do not probe every service host process.
